...
class ReviewsServices {
  ...
  async reviewMember(reviews: ReviewMember['reviews']) {
    const arrayReviews = reviews.map((item) => databaseService.query(`Insert into \`${DatabaseTable.Review}\`(reviewID,rate,description,receiverID,createdAt,updatedAt) values(?,?,?,?,?,?)`, handleSpreadObjectToArray(new Review({ ...item }))))
